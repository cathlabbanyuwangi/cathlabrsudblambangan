    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-slate-500 font-semibold mt-0.5">Daftarkan akun operasional sistem baru dan tentukan hak akses jabatannya.</p>
            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4 bg-slate-50/70 p-6 rounded-2xl border border-slate-200/60">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="name" class="block w-full text-xs rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-2xs py-2.5" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" placeholder="Contoh: dr. Ahmad Sp.JP" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="username" class="block w-full text-xs rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-2xs py-2.5" type="text" name="username" :value="old('username')" required autocomplete="username" placeholder="Contoh: ahmad_cbi" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="email" class="block w-full text-xs rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-2xs py-2.5" type="email" name="email" :value="old('email')" required autocomplete="email" placeholder="Contoh: ahmad@cathlab.test" />
                                    <x-input-error :messages="$errors->get('email')" class="mt-1" />
                                </div>
                            </div>

                            <!-- KOLOM KANAN: Keamanan & Hak Akses Role -->
                            <div class="space-y-4 bg-slate-50/70 p-6 rounded-2xl border border-slate-200/60">
                                <div class="flex items-center space-x-2 pb-2 border-b border-slate-200/60">
                                    ...
                                    <h3 class="text-xs font-black text-slate-800 uppercase tracking-wider">Keamanan & Jabatan</h3>
                                </div>

                                <!-- Role / Jabatan (Custom Alpine.js Dropdown yang Estetik & Rounded) -->
                                <div x-data="{ open: false, selected: '{{ old('role', '') }}', selectedLabel: '{{ old('role') ? ucwords(str_replace('-', ' ', old('role'))) : '-- Pilih Role / Jabatan --' }}' }" class="relative">
                                    ...
                                    <div x-show="open" 
                                        ...
                                        <div @click="selected = ''; selectedLabel = '-- Pilih Role / Jabatan --'; open = false" 
                                             class="px-3 py-2 text-xs text-slate-400 hover:bg-slate-50 cursor-pointer font-semibold">
                                            -- Pilih Role / Jabatan --
                                        </div>
                                ...
                                <!-- Password -->
                                <div>
                                    <x-input-label for="password" :value="__('Password')" class="text-[11px] font-bold uppercase tracking-wider text-slate-600 mb-1" />
                                    <x-text-input id="password" class="block w-full text-xs rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-2xs py-2.5" type="password" name="password" required autocomplete="new-password" placeholder="Minimal 8 karakter" />
                                    <x-input-error :messages="$errors->get('password')" class="mt-1" />
                                </div>

                                <!-- Confirm Password -->
                                <div>
                                    <x-input-label for="password_confirmation" :value="__('Konfirmasi Password')" class="text-[11px] font-bold uppercase tracking-wider text-slate-600 mb-1" />
                                    <x-text-input id="password_confirmation" class="block w-full text-xs rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-2xs py-2.5" type="password" name="password_confirmation" required autocomplete="new-password" placeholder="Ulangi password di atas" />
